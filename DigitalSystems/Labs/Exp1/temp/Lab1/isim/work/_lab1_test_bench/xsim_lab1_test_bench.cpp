...
va_list vap)
{
    extern HSim__s6 * createworkM_lab1_test_bench(const char*);
    HSim__s6 *blk = createworkM_lab1_test_bench(label); 
    return blk;
}
...
va_list vap)
{
    extern HSim__s6 * createworkM_lab1(const char*);
    HSim__s6 *blk = createworkM_lab1(label); 
    return blk;
}
...
public:
    ...
    HSimConfigDecl * topModuleInstantiate() {
        ...
        (*cfgvh).addVlogModule("work","Lab1TestBench", (HSimInstFactoryPtr)IF0);
        (*cfgvh).addVlogModule("unisims_ver","AND2", (HSimInstFactoryPtr)IF1);
        (*cfgvh).addVlogModule("unisims_ver","AND2", (HSimInstFactoryPtr)IF2);
        (*cfgvh).addVlogModule("work","Lab1", (HSimInstFactoryPtr)IF3);
        (*cfgvh).addVlogModule("work","glbl", (HSimInstFactoryPtr)IF4);
        HSim__s5 * topvl = 0;
        extern HSim__s6 * createworkM_lab1_test_bench(const char*);
        topvl = (HSim__s5*)createworkM_lab1_test_bench("Lab1TestBench");
