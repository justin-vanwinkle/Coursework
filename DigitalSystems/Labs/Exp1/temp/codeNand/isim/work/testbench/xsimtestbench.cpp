...
va_list vap)
{
    extern HSim__s6 * createworkMtestbench(const char*);
    HSim__s6 *blk = createworkMtestbench(label); 
    return blk;
}
...
va_list vap)
{
    extern HSim__s6 * createworkMc_nand(const char*);
    HSim__s6 *blk = createworkMc_nand(label); 
    return blk;
}
...
public:
    ...
    HSimConfigDecl * topModuleInstantiate() {
        ...
        (*cfgvh).addVlogModule("work","cNand", (HSimInstFactoryPtr)IF1);
        (*cfgvh).addVlogModule("work","glbl", (HSimInstFactoryPtr)IF2);
        HSim__s5 * topvl = 0;
        extern HSim__s6 * createworkMtestbench(const char*);
        topvl = (HSim__s5*)createworkMtestbench("testbench");
