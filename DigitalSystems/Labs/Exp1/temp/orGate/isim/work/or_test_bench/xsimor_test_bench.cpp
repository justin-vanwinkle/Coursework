...
va_list vap)
{
    extern HSim__s6 * createworkMor_test_bench(const char*);
    HSim__s6 *blk = createworkMor_test_bench(label); 
    return blk;
}
...
va_list vap)
{
    extern HSim__s6 * createworkMor_gate(const char*);
    HSim__s6 *blk = createworkMor_gate(label); 
    return blk;
}
...
public:
    ...
    HSimConfigDecl * topModuleInstantiate() {
        ...
        (*cfgvh).addVlogModule("work","orTestBench", (HSimInstFactoryPtr)IF0);
        (*cfgvh).addVlogModule("unisims_ver","OR2", (HSimInstFactoryPtr)IF1);
        (*cfgvh).addVlogModule("unisims_ver","OR2", (HSimInstFactoryPtr)IF2);
        (*cfgvh).addVlogModule("work","orGate", (HSimInstFactoryPtr)IF3);
        (*cfgvh).addVlogModule("work","glbl", (HSimInstFactoryPtr)IF4);
        HSim__s5 * topvl = 0;
        extern HSim__s6 * createworkMor_test_bench(const char*);
        topvl = (HSim__s5*)createworkMor_test_bench("orTestBench");
