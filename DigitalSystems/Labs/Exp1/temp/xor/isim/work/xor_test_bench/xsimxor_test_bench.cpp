...
va_list vap)
{
    extern HSim__s6 * createworkMxor_test_bench(const char*);
    HSim__s6 *blk = createworkMxor_test_bench(label); 
    return blk;
}
...
va_list vap)
{
    extern HSim__s6 * createworkMxorsch(const char*);
    HSim__s6 *blk = createworkMxorsch(label); 
    return blk;
}
...
public:
    ...
    HSimConfigDecl * topModuleInstantiate() {
        ...
        (*cfgvh).addVlogModule("work","xorTestBench", (HSimInstFactoryPtr)IF0);
        (*cfgvh).addVlogModule("unisims_ver","XOR2", (HSimInstFactoryPtr)IF1);
        (*cfgvh).addVlogModule("unisims_ver","XOR2", (HSimInstFactoryPtr)IF2);
        (*cfgvh).addVlogModule("work","xorsch", (HSimInstFactoryPtr)IF3);
        (*cfgvh).addVlogModule("work","glbl", (HSimInstFactoryPtr)IF4);
        HSim__s5 * topvl = 0;
        extern HSim__s6 * createworkMxor_test_bench(const char*);
        topvl = (HSim__s5*)createworkMxor_test_bench("xorTestBench");
