...
va_list vap)
{
    extern HSim__s6 * createworkMinv_test_bench(const char*);
    HSim__s6 *blk = createworkMinv_test_bench(label); 
    return blk;
}
...
va_list vap)
{
    extern HSim__s6 * createworkMinvsch(const char*);
    HSim__s6 *blk = createworkMinvsch(label); 
    return blk;
}
...
public:
    ...
    HSimConfigDecl * topModuleInstantiate() {
        ...
        (*cfgvh).addVlogModule("work","invTestBench", (HSimInstFactoryPtr)IF0);
        (*cfgvh).addVlogModule("unisims_ver","INV", (HSimInstFactoryPtr)IF1);
        (*cfgvh).addVlogModule("unisims_ver","INV", (HSimInstFactoryPtr)IF2);
        (*cfgvh).addVlogModule("work","invsch", (HSimInstFactoryPtr)IF3);
        (*cfgvh).addVlogModule("work","glbl", (HSimInstFactoryPtr)IF4);
        HSim__s5 * topvl = 0;
        extern HSim__s6 * createworkMinv_test_bench(const char*);
        topvl = (HSim__s5*)createworkMinv_test_bench("invTestBench");
