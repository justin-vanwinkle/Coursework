...
va_list vap)
{
    extern HSim__s6 * createworkMpart4_test_bench(const char*);
    HSim__s6 *blk = createworkMpart4_test_bench(label); 
    return blk;
}
...
va_list vap)
{
    extern HSim__s6 * createworkMtwo_in_five_out(const char*);
    HSim__s6 *blk = createworkMtwo_in_five_out(label); 
    return blk;
}
...
public:
    ...
    HSimConfigDecl * topModuleInstantiate() {
        ...
        (*cfgvh).addVlogModule("work","part4TestBench", (HSimInstFactoryPtr)IF0);
        (*cfgvh).addVlogModule("work","twoInFiveOut", (HSimInstFactoryPtr)IF1);
        (*cfgvh).addVlogModule("work","glbl", (HSimInstFactoryPtr)IF2);
        HSim__s5 * topvl = 0;
        extern HSim__s6 * createworkMpart4_test_bench(const char*);
        topvl = (HSim__s5*)createworkMpart4_test_bench("part4TestBench");
